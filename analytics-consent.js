(function () {
  ...
  var state = null, ready = false, sent = {}, reopened = false;
  ...
  function choose(status) { var wasAccepted = state && state.status === "accepted"; save(status); if(status === "accepted") load(); else { gtag("consent","update",{analytics_storage:"denied",ad_storage:"denied",ad_user_data:"denied",ad_personalization:"denied"}); removeCookies(); } close(); if(status === "denied" && reopened && wasAccepted) window.location.reload(); }
  function banner() { var b=document.createElement("aside"); b.id="analytics-banner"; b.className="analytics-banner"; b.setAttribute("role","dialog"); b.setAttribute("aria-labelledby","analytics-title"); b.innerHTML='<div><h2 id="analytics-title">'+text("Statistiche del sito","Website analytics")+'</h2><p>'+text("Con il tuo consenso utilizziamo Google Analytics per capire come viene utilizzato il sito e migliorarlo. Il sito funziona anche senza accettare.","With your consent, we use Google Analytics to understand how the site is used and improve it. The website works normally even if you decline.")+'</p><a href="privacy.html">'+text("Privacy e cookie","Privacy and cookies")+'</a></div><div class="analytics-actions"><button type="button" data-analytics="deny">'+text("Rifiuta","Decline")+'</button><button type="button" data-analytics="accept">'+text("Accetta","Accept")+'</button></div>'; document.body.appendChild(b); b.addEventListener("click",function(e){var a=e.target.getAttribute("data-analytics"); if(a) choose(a === "accept" ? "accepted":"denied");}); }
  function prefs() { var b=document.getElementById("analytics-banner"); if(!b){ banner(); b=document.getElementById("analytics-banner"); } reopened=true; b.hidden=false; }
